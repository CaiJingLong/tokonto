...
const archive = join(output, `${name}.tar.gz`), stage = mkdtempSync(join(tmpdir(), 'token-usage-pack-'));
try {
  ...
  for (const path of ['README.md', 'CHANGELOG.md', 'SECURITY.md', 'CONTRIBUTING.md', 'docs', 'examples', '.agents/skills/token-usage']) {
  ...
  await Bun.write(join(root, 'package.json'), JSON.stringify({
    ...
    engines: manifest.engines, bin: { 'token-usage': 'cli.js' },
  }, null, 2) + '\n');
