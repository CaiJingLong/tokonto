...
const required = ['dist/cli.js', 'dist/web/index.html', 'dist/web/app.js', 'dist/web/style.css', 'dist/skill/SKILL.md', 'dist/THIRD_PARTY_NOTICES.md', 'LICENSE', 'README.md', 'README.en.md'];
for (const path of required) if (!info.files.some((f: any) => f.path === path)) throw new Error(`Missing package asset: ${path}`);
const allowed = /^(?:dist\/|docs\/|examples\/|\.agents\/skills\/tokonto\/|(?:package\.json|README(?:\.en)?\.md|LICENSE|CHANGELOG\.md|CONTRIBUTING\.md|SECURITY\.md)$)/;
