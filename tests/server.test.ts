...
test('dashboard shares CLI logic and rejects cross-origin mutations', async () => {
  ...
  try {
    ...
    const html = await fetch(base).then(r => r.text()); expect(html).toContain('Tokonto');
    const query = (command: string, input: any, headers = {}) => fetch(base + 'api/command', { method: 'POST', headers: { 'content-type': 'application/json', 'x-tokonto': '1', ...headers }, body: JSON.stringify({ command, input }) });
    ...
    const legacyHeaders = { 'x-tokonto': '', 'x-token-usage': '1' };
    expect((await query('usage stats', {}, legacyHeaders)).status).toBe(200);
    expect((await query('sync', {}, { ...legacyHeaders, origin: 'https://evil.example' })).status).toBe(403);
