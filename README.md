...
需要 **Bun ≥ 1.3.14**（[安装 Bun](https://bun.sh/docs/installation)）。从仓库 Releases 下载 `token-usage-0.1.0.tar.gz` 和 `SHA256SUMS`，放在同一目录：
...
无需安装项目依赖。发行包包含 CLI、Dashboard、Skill 和使用文档，运行仍需 Bun。使用 `bun link` 后可调用 `token-usage`；未链接时用 `bun /安装目录/cli.js` 代替本文中的命令。Bun 的 bin 目录需在 PATH 中。
...
本文后续用 `token-usage` 表示 CLI。可以通过 `bun link` 链接命令，也可以始终用 `bun run cli --` 代替。数据库默认位于 `~/.token-usage/usage.sqlite`；所有命令均支持 `--data-dir /path/to/data` 或环境变量 `TOKEN_USAGE_HOME`。默认来源只初始化一次；预设价格随程序提供，不会覆盖用户配置和已存费用。
...
数据库只保存白名单内的用量元数据，不保存聊天正文、API key 或原始日志。日志文件仍属于原应用，始终只读。

```sh
token-usage doctor --json
token-usage providers list --json
token-usage sync --id omp --json
token-usage sync --force --json
```
...
```sh
# 示例含基准价格与指定一周的凌晨半价
token-usage prices put --input @examples/prices.json --dry-run --json
token-usage prices put --input @examples/prices.json --json

# 规则历史、单条费用解释与全部修订历史
token-usage prices list --history --json
token-usage prices explain --source omp --id '<event-id>' --json
token-usage prices history --source omp --id '<event-id>' --json

# 重算默认只预览；应用时增加 apply=true
token-usage prices reprice --input '{"query":{"source":"omp"}}' --json
token-usage prices reprice --input '{"query":{"source":"omp"},"apply":true}' --json
...
```json
{
  "provider": {
    ...
    "name": "My Agent",
    "kind": "script",
    "command": ["/absolute/path/to/bun", "/absolute/token-usage/examples/provider.mjs", "/absolute/token-usage/examples/agent.jsonl"]
  }
}
```

```sh
token-usage providers put --input @provider.json --json
token-usage providers test --id my-agent --json
token-usage sync --id my-agent --json
token-usage sync --id my-agent --json  # 第二次不会重复累计
```

## AI 优先：Skill + CLI

项目 Skill 位于 [.agents/skills/token-usage/SKILL.md](.agents/skills/token-usage/SKILL.md)。它可以随项目加载，也可以安装到工具的个人技能目录：

```sh
token-usage skill install --target ~/.codex/skills --json
token-usage skill install --target ~/.omp/agent/skills --json
```
...
```sh
token-usage --version --json
token-usage schema --json
token-usage schema --command 'prices put' --json
token-usage usage stats --input '{"source":"omp","from":"2026-09-01T00:00:00+08:00","to":"2026-10-01T00:00:00+08:00","timezone":"Asia/Shanghai","groupBy":"hour"}' --json
```
...
发行包构建与独立安装检查：`bun run release:pack`、`bun run release:smoke`。GitHub Actions 在 macOS、Linux 运行同样检查；版本 tag 通过验证后生成 Release 草稿。详见[发布文档](docs/releasing.md)和[更新记录](CHANGELOG.md)。
...
```sh
bun scripts/demo.ts /tmp/token-usage-demo
bun run cli -- server --data-dir /tmp/token-usage-demo --port 4320 --interval 0
```
...
升级前停止 server 和所有访问账本的 CLI，再复制整个 `~/.token-usage` 目录（自定义 `--data-dir` 则复制对应目录）到安全位置，包括可能存在的 SQLite WAL 文件。不要在写入期间仅复制 `usage.sqlite`。

将新版本解压到新目录，使用同一数据目录启动；默认配置只初始化一次，已有费用不会因升级预设而自动重算。首次启动可能创建统计索引，大账本需要额外时间和磁盘空间。需要回退时先停止服务，恢复完整升级前备份，再启动旧版本。

卸载时停止服务，若执行过 `bun link`，在对应安装目录执行 `bun unlink`，然后删除安装目录。账本和另行安装的 Skill 会保留，按需要单独备份或移除。
